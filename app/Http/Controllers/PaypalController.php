...
//        dd($invoice_id);
       ...
       //dd($request);
        ...
        $cart['items'][0]['price'] = $request->price;
        // create new invoice
        if($request->recurring_name != "recurringname") {
            $invoice = new Invoice();
            $invoice->service_id = $request->project_id;
            $invoice->service_title = $request->name;
...
//        dd($response);
    ...
    private function getCart($recurring, $invoice_id)
    {
        ...
        return [
            ...
            'invoice_id' => config('paypal.invoice_prefix') . '_' . $invoice_id,
            'invoice_description' => "Order #" . $invoice_id . " Invoice",
            'cancel_url' => url('/paypalredirection/'.$invoice_id),
            // total is calculated by multiplying price with quantity of all cart items and then adding them up
    ...
    public function expressCheckoutSuccess(Request $request) {
        ...
        }if($invoice->service_title == 'pel_service') {
            $service = Pel::find($invoice->service_id);
        }

        Auth::loginUsingId($service->user_id);
        $cart = $this->getCart($recurring, $invoice_id);
//        dd($cart['total']);
        $cart['total'] = $invoice->price;
//        dd($cart['total']);
        $cart['items'][0]['price'] = $invoice->price;
...
//        dd($invoice_id);
        ...
        if ($invoice->paid) {
           Mail::to(Auth::user()->email)->cc('info@bullseyelocting.com')->send(new NewUserbyForm($invoice));
//            return redirect(('/invoice/'.$invoice->id))->with(['code' => 'success', 'message' => 'Order ' . $invoice->id . ' has been paid successfully!']);
            return redirect(('/home'))->with(['code' => 'success', 'message' => 'Order ' . $invoice->id . ' has been paid successfully!']);
        }
